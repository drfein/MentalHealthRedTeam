...
\begin{enumerate}
    ...
    \item \textbf{An explicit endpoint-sensitivity analysis.} A framing-aware rubric and the exact SPIRALS assistant-endorsement rubric agree on 19 direct-response positives but differ substantially in absolute rate, documenting what each operationalization supports.
\end{enumerate}
...
\begin{figure*}[t]
    ...
    \caption{Full-benchmark controlled evaluation of \model{}. \textbf{A:} Assistant endorsement rates for all 433 target turns under each frame. \textbf{B:} Paired target-level risk differences between direct assertion and each alternative frame. Intervals are 95\% source-conversation cluster-bootstrap intervals over 232 source conversations.}
    \label{fig:main}
\end{figure*}

\subsection{Absolute endorsement depends on the rubric}

The exact SPIRALS package rubric marks 20/433 direct responses positive (4.6\%, 95\% source-conversation cluster-bootstrap CI [2.5, 7.1]). Nineteen are also positive under the framing-aware rubric; one is package-only, while 56 are framing-only. The package rubric demands stronger, more explicit assistant endorsement and was not designed to compare translation or fiction frames. The paired framing result is therefore the primary analysis, while the package count shows that the absolute direct-assertion rate is operationalization-dependent.
...
The framing-aware endpoint is intentionally sensitive to accepting or elaborating a claim as real, whereas the package endpoint is stricter. Their discrepancy rules out presenting 17.3\% as an endpoint-free failure rate. The robust automatic conclusion is comparative: under the framing-aware rubric, direct assertion is the condition most likely to elicit agreement. Human adjudication and replication across model families remain necessary before treating either automatic rate as a model-level safety score.
...
\section{Exploratory J-space Negative Result}
\label{app:additional}

We explored whether a public Jacobian lens for \model{} \citep{qwen2025technical,gurnee2026jspace} could predict the model's direct-assertion behavior at the assistant-response boundary. For vocabulary token $w$, the readout was the maximum logit over prompt positions, $m_{\ell,w}=\max_t z_{\ell,t,w}$. A discovery analysis selected the inverse layer-26 \texttt{misinformation} coordinate. This analysis is appendix-only because the original target-level split allowed source-conversation overlap, the corrected interval includes chance, and the named coordinate failed semantic-specificity controls.
