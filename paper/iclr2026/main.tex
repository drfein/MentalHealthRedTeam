...
\begin{enumerate}
    \item \textbf{A context-verified wild benchmark.} \dataset{} contains 433 LLM-filtered target turns from 232 source conversations in two redistributable open corpora, produced by a reproducible rare-event retrieval and conversation-verification pipeline with explicit exclusion reasons and source-license filtering.
    \item \textbf{A full-benchmark paired evaluation.} Eight deterministic speech-act templates hold claim text fixed while contrasting direct assertion with seven alternative tasks across all 433 released targets; direct assertion has judged endorsement at least 14.3 percentage points higher than every alternative frame.
...
\begin{figure*}[t]
    ...
    \caption{Full-benchmark paired-template evaluation of \model{} under two automatic rubric formulations. \textbf{A:} Judged assistant-endorsement rates for all 433 target turns under each frame. \textbf{B:} Paired target-level risk differences between direct assertion and each alternative frame. Intervals are 95\% source-conversation cluster-bootstrap intervals over 232 source conversations.}
    \label{fig:main}
\end{figure*}
...
The framing-aware direct-assertion rate is also descriptively highest within each released source stratum (Appendix Table~\ref{tab:source-behavior}). The WildChat and ShareChat--Grok strata are small, so this check shows directional consistency in these data rather than source-specific effect estimates.
...
We introduce \dataset{}, a context-preserving benchmark of 433 target turns from 232 source conversations, mined from 6.13M user messages with a reproducible rare-event retrieval and verification pipeline. Its natural claims support controlled evaluations that would be difficult to construct from synthetic prompts alone: across all released targets, direct user assertion has the highest judged-endorsement rate among eight matched templates. The central contribution is the reusable wild-data benchmark and a full-benchmark paired evaluation with transparent endpoint sensitivity.
